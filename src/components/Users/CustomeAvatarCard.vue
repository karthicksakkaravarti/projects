        ...
        <v-menu open-on-hover offset-x v-model="menu" :close-on-click="false"
            ...
            <template v-slot:activator="{ on }">
                <div class="d-flex" v-on="on" @click="showProfile">
                    <v-avatar class="ma-0 pa-0" size="28">
                        <v-img v-if="details.profile_picture" :src="details.profile_picture">
                        </v-img>
                        <v-img v-else ></v-img>
                    </v-avatar>

                    <div>
                        <span class="ml-2" color="primary">{{ details.full_name }}</span>
                    </div>
                </div>
            </template>
            ...
            <v-card width="350" height="190">
                <v-list>
                    <v-list-item>
                        <v-list-item-avatar>
                            <img v-if="details.profile_picture" :src="details.profile_picture" />
                            <img v-else  />
                        </v-list-item-avatar>
                        <v-list-item-content>
                            <v-list-item-title>{{ details.full_name }}</v-list-item-title>
                ...
                <div class="d-flex justify-start ma-2">
                    <a :href="`MSTeams:/l/chat/0/0?users=${details.username}@gmail.com`">
                    ...
                    <a class="ml-2" :href="`mailto:${details.email}`"><svg xmlns="http://www.w3.org/2000/svg" width="24"
...
export default {
    ...
    methods: {
        ...
        showProfile() {
            if (this.details.employee_id) {
                this.$router.push({ name: 'RM User View', params: { id: this.details.employee_id } })
            }
    }
    }
